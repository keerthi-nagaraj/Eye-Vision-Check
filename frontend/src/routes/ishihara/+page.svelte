<script lang="ts">
	import IshiharaPlate from '$lib/components/IshiharaPlate.svelte';
	...
	function generateRandomPlates() {
		...
		for (let i = 0; i < PLATE_COUNT; i++) {
			const num = Math.floor(Math.random() * (NUMBER_MAX - NUMBER_MIN) + NUMBER_MIN);
			plates.push({ number: num, expected: num.toString() });
		}
		return plates;
	}
	...
	*/
	...
	$: currentPlate = testPlates[currentPlateIndex];
	...
	*/
	...
	function startTest() {
		if (visionReady) {
			testPlates = generateRandomPlates();
			testStarted = true;
		}
	}
	...
	function handleEnterKey(event: KeyboardEvent) {
		if (event.key === 'Enter') {
			// Save answer
			userAnswers[currentPlateIndex] = answer.trim();

			// Send answer data to backend via WebSocket - commented out
			/*
			if (ws && wsConnected) {
				try {
					const correct = (answer.trim() === currentPlate.expected);
					ws.send(JSON.stringify({
						type: 'ishihara_answer',
						plateNumber: currentPlateIndex + 1,
						expected: currentPlate.expected,
						userAnswer: answer.trim(),
						correct: correct,
						distance: visionDistance || 0
					}));
				} catch (error) {
					console.error('Failed to send Ishihara answer data:', error);
				}
			}
			*/

			// Move to next plate or complete test
			if (currentPlateIndex < testPlates.length - 1) {
				...
				sendIshiharaCompletion();
			}
		}
	}

	function sendIshiharaCompletion() {
		...
		if (ws && wsConnected) {
			try {
				ws.send(JSON.stringify({
					type: 'ishihara_complete',
					totalPlates: testPlates.length,
					correct: userAnswers.filter((ans, i) => ans.trim() === testPlates[i].expected).length,
					score: score,
					diagnosis: resultText,
					distance: visionDistance || 0
				}));
			} catch (error) {
				console.error('Failed to send Ishihara completion data:', error);
			}
		}
		*/
	}
	...
	function calculateScore() {
		...
		return Math.round(
			(correct / testPlates.length) * 100
		);
	}

	function getResultText(score: number) {
		if (score >= 80) {
...
<div class="container">
	{#if !testStarted && !testEndedDueToNoFace}
		...
		<p class="subtitle">Color vision test with Ishihara plates</p>
	...
	{:else if !testCompleted}
		...
		<div class="plate-info">
			<span>Plate {currentPlateIndex + 1} / {testPlates.length}</span>
		</div>
		...
		<input
			...
			bind:value={answer}
			placeholder="Enter the number you see (Press Enter to continue)"
	...
	{:else}
		...
		<div class="results-container">
			<div class="score-display">
				...
				<p class="result-text">{getResultText(calculateScore())}</p>
			</div>

			<div class="answers-summary">
				{#each testPlates as plate, i}
					<div class="answer-item">
						<span class="plate-num">Plate {i + 1}:</span>
						<span class="expected">Expected: {plate.expected}</span>
						<span class="user-ans">Your answer: {userAnswers[i] || 'No answer'}</span>
						{#if (userAnswers[i] || '').trim() === plate.expected}
